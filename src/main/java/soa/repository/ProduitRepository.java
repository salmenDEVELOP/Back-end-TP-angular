...
//	@Query("SELECT p FROM Produit p WHERE p.categorie.nom LIKE :categorie AND p.prix > :prix")
//	public List<Produit> findByCategorieAndPrix(@Param("x") String mc, @Param("y") double prix);
  ...
  //Retourner la liste de tous les produits en ordre croissant selon le prix et prix > prix_min
   List<Produit> findByPrixGreaterThanOrderByPrixAsc(double prixMin);
	...
	List<Produit> findAllByCategorieAndPrixGreaterThan(Categorie categorie,double prix);
}
